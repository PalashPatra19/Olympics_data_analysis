...
def participating_regions_over_time(df):
    # Calculate unique year-region combinations and get their counts
    nations_over_time = df.drop_duplicates(['Year','region'])['Year'].value_counts().reset_index()

    # Rename columns to make sorting easier
...
def most_successful(df, sport):
    ...
    x = x[['Name', 'Medals', 'Sport', 'region']].head(15).reset_index(drop=True)
    return x
...
def most_successful_countrywise(df, country):
    ...
    x.rename(columns={'Name': 'Name', 'count': 'Medals'}, inplace=True)
    x = x[['Name', 'Medals', 'Sport',]].head(15).reset_index(drop=True)
    return x
